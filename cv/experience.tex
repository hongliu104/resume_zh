...
%---------------------------------------------------------
  \cventry
    {高级软件主管工程师} % Job title
    {上海燧原科技有限公司} % Organization
    {成都} % Location
    {2021.07 - 至今} % Date(s)
    {   
      \begin{itemize}[parsep=0.2ex] % Description(s) of tasks/responsibilities
        \item \descriptionstyle{带领4人团队研发hlir\_builder(类xla\_builder), 提供图编译的上层op的C++/Python接口，实现接口自动生成，并与内部mlir解耦;}
        \item \descriptionstyle{支持图模式对接主流训练/推理框架：pytorch, tensorflow, onnx, paddlepaddle, mindspore, oneflow;}
        \item \descriptionstyle{主力对接oneflow, oneflow-xrt, libai, 支持大模型训练如vit;}
      \end{itemize}
    }
